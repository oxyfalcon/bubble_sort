...
class SwapNotifier extends ChangeNotifier {
  Stream<((Offset, double), (Offset, double))> fetch(
      List<(Offset, double)> centers, Duration duration) async* {
    for (int i = 0; i < centers.length - 1; i++) {
      for (int j = 0; j < centers.length - i - 1; j++) {
        if (centers[j].$2 > centers[j + 1].$2) {
          notifyListeners();
          yield await Future.delayed(
              duration, () => (centers[j], centers[j + 1]));
...
class _MyAppState extends State<MyApp> with TickerProviderStateMixin {
  ...
  void callBack() => WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
        setState(() {
          controller.forward();
        });
      });
  ...
  Stream<((Offset, double), (Offset, double), List<(Offset, double)>)> fetch(
      List<(Offset, double)> centers) async* {
    for (int i = 0; i < centers.length - 1; i++) {
                                      ...
                                      return RepaintBoundary(
                                        child: CustomPaint(
                                          ...
                                          painter: ShapePainter(
                                              ...
                                              callback: callBack,
                                              heightFactorList: heightFactors),
                                        ),
                                      );
                                    });
                              })),
